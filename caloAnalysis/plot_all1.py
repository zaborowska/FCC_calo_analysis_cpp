...
def truncated_mean(histo,trunctation) :
    ...
    print "Truncated Mean Ecell %5.2f " % histo.GetMean(), " RMS %5.2f " % histo.GetRMS() 
...
SUFFIX = ["1.5-1.7_more","0.6-0.8_more","15_more"]
#SUFFIX = ["1.5-1.5_more","15_more"]
...
for i in range(len(SUFFIX)):
   ...
   # print "Plots from file: ", filename0[i]
    print "Plots from file: ", filename1[i]
...
for i in range(len(longprofile_1)):
    c0.cd(1)
    leg.AddEntry(longprofile_1[i],SUFFIX[i],"lep")
    longprofile_1[i].SetLineColor(i+1)
    longprofile_1[i].GetXaxis().SetTitle("Rho [mm]")
    longprofile_1[i].GetXaxis().SetTitle("Energy/event in the layer [GeV]")
    ...
    leg.Draw()
    c0.cd(2)
    cellene_1[i].Rebin(10)
    cellene_1[i].SetLineColor(i+1)
    cellene_1[i].SetAxisRange(0.,ENERGY/10.)
    cellene_1[i].GetXaxis().SetTitle("Total cell energy [GeV]")
    if i==0:
       cellene_1[i].Draw()
    else:
...
#for i in range(len(cellene_1)):
#   leg.AddEntry(cellene_1[i],SUFFIX[i],"lep")
   # c0.cd(1)
...
c0.SaveAs("c_"+PARTICLE+str(ENERGY)+"_moreevents.png")
